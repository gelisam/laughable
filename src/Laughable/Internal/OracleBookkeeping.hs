...
--     test o = do
--       putStr $ show (o ^.. past . each . answer)
--       putStr " "
...
-- >>> let Left s1 = runOracleMachine oracleMachine         & _Left . answer .~ 10
-- >>> let Left s2 = runOracleMachine (resumeWithAnswer s1) & _Left . answer .~ 20
-- >>> let Left s3 = runOracleMachine (resumeWithAnswer s2) & _Left . answer .~ 30
--
...
-- >>> let Just b2' = b3    & left (\s a -> (succ a, s ^. answer))
-- >>> let Just b1' = b2'   & left (\s a -> (succ a, s ^. answer))
-- >>> let Just b0' = b1'   & left (\s a -> (succ a, s ^. answer))
-- >>> let b1''     = b0'   & right (\a (Just 10) -> (s1, succ a))
-- >>> let b2''     = b1''  & right (\a (Just 20) -> (s2, succ a))
-- >>> let b3''     = b2''  & right (\a (Just 30) -> (s3, succ a))
